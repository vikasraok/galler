...
// Returns the local name of a node.  Needed for older versions of ie
        ...
        $.widget("xml.xmlEditor", {
            options: {
                ...
                // Function triggered after uploading XML document, to interpret if the response was successful or not.
                //  If upload failed, an error message should be returned.
                submitResponseHandler: null,
                // Function triggered after uploading XML document, if an error occurs. Gives full text of error,
                // instead of a boilerplate "500 server error" message.
            ...
            _create: function () {
                ...
                if (typeof(
                    ...
                    if ((
                        typeof schema == 'string' || typeof schema instanceof String)) {
            ...
            _init: function () {
                if (this.options.submitButtonConfigs) {
                    ...
                    this.submitButtonConfigs = [{
                        ...
                        label: exporting ? "Export" : "Submit",
                        onSubmit: exporting ? this.exportXML : null,
                        disabled: typeof(
                            Blob) === undefined && exporting
                    }];
                }
                ...
                if (this.element.is("textarea")) {
                    ...
                    $(this.element).before(this.xmlEditorContainer).hide();
                } else {
            ...
            loadDocument: function (ajaxOptions, localXMLContent) {
                ...
                if (ajaxOptions != null && ajaxOptions.xmlRetrievalPath != null) {
                    ...
                    $.ajax({
                        ...
                        data: (
                            ajaxOptions.xmlRetrievalParams),
                        dataType: "text",
                        success: function (data) {
            ...
            constructEditor: function () {
                ...
                this.xmlWorkAreaContainer =
                    $("<div/>").attr('class', xmlWorkAreaContainerClass).appendTo(this.xmlEditorContainer);

                // Menu bar
                var editorHeaderBacking = $("<div/>").addClass(editorHeaderClass + "_backing").
                    appendTo(this.xmlWorkAreaContainer);
                ...
                this.xmlTabContainer =
                    $("<div/>").attr("class", editorTabAreaClass).css("padding-top", this.editorHeader.height() + "px").
                        appendTo(this.xmlWorkAreaContainer);
                this.problemsPanel =
                    $("<pre/>").attr('class', problemsPanelClass).hide().appendTo(this.xmlTabContainer);
            ...
            resize: function () {
                this.xmlTabContainer.width(
                    this.xmlEditorContainer.outerWidth() - this.modifyMenu.menuColumn.outerWidth());
            ...
            addChildElement: function (parentElement, newElementDefinition, relativeTo, prepend) {
                ...
                if (typeof newElementDefinition == 'string' || newElementDefinition instanceof String) {
                    ...
                    if (!objectType && !parentElement.objectType.any) {
                        return "Could not add child " + newElementDefinition + ", it is not a valid child of " +
                            parentElement.objectType.localName;
                    }
                } else {
            ...
            addAttribute: function (xmlElement, attrDefinition, instigator) {
                ...
                if ($.type(attrDefinition) === "object") {
                    ...
                    if (!objectType && !xmlElement.objectType.anyAttribute) {
                        return "Could not add attribute " + attrDefinition + ", it is not a valid for element " +
                            xmlElement.objectType.localName;
                    }
                }
            ...
            addNode: function (parentElement, nodeType, prepend, relativeTo) {
                ...
                if (!(
                    parentElement instanceof XMLElement)) {
                    return;
                }
            ...
            modeChange: function (mode) {
                ...
                if ((
                    mode == 0 && this.guiEditor.active) || (
                    mode == 1 && this.textEditor.active))
                    return this;
                ...
                if (mode == 0) {
                    this.activeEditor = this.guiEditor;
                    $("#" + xmlMenuHeaderPrefix + this.options.xmlEditorLabel.replace(/ /g, "_")).
                        addClass("active_mode_tab");
                } else {
                    this.activeEditor = this.textEditor;
                    $("#" + xmlMenuHeaderPrefix + this.options.textEditorLabel.replace(/ /g, "_")).
                        addClass("active_mode_tab");
                }
            ...
            refreshDisplay: function () {
                ...
                this.xmlWorkAreaContainer.width(
                    this.xmlEditorContainer.outerWidth() - this.modifyMenu.menuColumn.outerWidth());
            },
            ...
            exportXML: function () {
                if (typeof(
                        Blob) === "undefined") {
                    this.addProblem(
                        "Browser does not support saving files via this editor.  To save, copy and paste the document from the Text view.");
                    return false;
                }

                var exportDialog = $(
                    "<form><input type='text' class='xml_export_filename' placeholder='file.xml'/><input type='submit' value='Export'/></form>").
                    dialog({
                        modal: true,
                        dialogClass: 'xml_dialog',
                        resizable: false,
                ...
                exportDialog.submit(function () {
                    if (self.textEditor.active) {
                        try {
                            ...
                            $("." + submissionStatusClass).html("Failed to save<br/>See errors at top").
                                css("background-color", "#ffbbbb").animate({backgroundColor: "#ffffff"}, 1000);
            ...
            uploadXML: function (config) {
                ...
                if (this.textEditor.active) {
                    try {
                        ...
                        $("." + submissionStatusClass).html("Failed to submit<br/>See errors at top").
                            css("background-color", "#ffbbbb").animate({backgroundColor: "#ffffff"}, 1000);
            ...
            swordSubmitResponseHandler: function (response) {
                ...
                if (responseObject.length > 0 &&
                    localName(responseObject[responseObject.length - 1]) == "sword:error") {
            ...
            xml2Str: function (xmlNodeObject) {
                ...
                var xmlNode = (
                    xmlNodeObject instanceof jQuery ? xmlNodeObject[0] : xmlNodeObject);
                var xmlStr = "";
            ...
            // Performs an equality comparison between two nodes, returning true if they match namespace uri and
            // element name
            ...
            keydownCallback: function (e) {
                ...
                if (this.guiEditor.active) {
                    ...
                    if (e.which > 36 && e.which < 41) {
                        if (e.altKey && (
                            focused.length == 0 || focused.is("textarea"))) {
                            e.preventDefault();
                    ...
                    if ((
                        e.metaKey || e.ctrlKey) && focused.length == 0 && e.which == 'Z'.charCodeAt(0)) {
                        // Undo
                        this.undoHistory.changeHead(e.shiftKey ? 1 : -1);
                        return false;
                    } else if ((
                        e.metaKey || e.ctrlKey) && focused.length == 0 && e.which == 'Y'.charCodeAt(0)) {
                        // Redo
            ...
            refreshMenuSelected: function (self) {
                ...
                $.each(suffixes, function () {
                    if (hasSelected)
                        $("#" + xmlMenuHeaderPrefix + this.toString()).removeClass("disabled").
                            data("menuItemData").enabled = true;
                    else $("#" + xmlMenuHeaderPrefix + this.toString()).addClass("disabled").
                        data("menuItemData").enabled = false;
                });
            },
            ...
            getVocabulary: function (xmlElement) {
                ...
                if (!matchingVocab || !(
                    matchingVocab in this.options.vocabularyConfigs.vocabularies)) {
                    return null;
                }
...
// appendTarget - DOM element which the new input will be append to
        AbstractXMLObject.prototype.createElementInput = function (inputID, startingValue, appendTarget) {
            ...
            if (this.objectType.values && this.objectType.values.length > 0) {
                ...
                this.objectType.type == 'string' || this.objectType.type == 'mixed'))
...
// Move this element down one location in the gui.  Returns true if the swap was able to happen
        ...
        AddNodeMenu.prototype.populate = function (xmlElement) {
            ...
            if (this.expanded) {
                ...
                if (endingHeight == 0)
                    endingHeight = 1;
                this.menuContent.css({height: startingHeight + "px"}).stop().
                    animate({height: endingHeight + "px"}, menuExpandDuration).show();
            }
        ...
        AttributeMenu.prototype.populate = function (xmlElement) {
            if (xmlElement == null || (
                this.target != null && xmlElement.domNode != null
                ...
                $(this.target.xmlNode[0].attributes).each(function () {
                    ...
                    $.each(attributesArray, function () {
                        if (this.name == targetAttribute.nodeName) {
                            attributesPresent[this.name] =
                                $("#" + xmlElement.domNodeID + "_" + targetAttribute.nodeName.replace(':', '-'));
                        }
                    });
                });
...
// The namespace will be recorded on the root element if possible
        DocumentState.prototype.addNamespace = function (prefixOrType, namespace) {
            ...
            if (prefix != null && !namespace) {
                namespace = (
                "urn:ns:local:xxxxxx-" + (
                new Date().getTime() % 0x1000000).toString(16)).replace(/x/g, function (c) {
                    return (
                    Math.random() * 16 | 0).toString(16);
                });
            } else if (namespace && (
                prefix == null || prefixExists)) {
...
// Deserialize a string representation of XML into an XML document
        ...
        function formatXML(element, indent, options) {
            ...
            if (attributes) {
                ...
                for (var index = 0; index < attributes.length; index++) {
                    ...
                    attrContents +=
                        " " + attributes[index].nodeName + '="' + escapeXml(attributes[index].nodeValue) + '"';
            ...
            if (element.nodeType == 1) {
                if (contents) {
                    var closingIndent = (
                        !containsText) ? "\n" + indent : "";
                    return indent + "<" + element.nodeName + attrContents + ">" + contents + closingIndent + "</" +
                        element.nodeName + ">";
                } else {
        ...
        GUIEditor.prototype.initialize = function (parentContainer) {
            ...
            this.placeholder = $("<div/>").attr("class", "placeholder").html(
                "There are no elements in this document.  Use the menu on the right to add new top level elements.").
                appendTo(this.xmlContent);

            this.guiContent =
                $("<div/>").attr({'id': guiContentClass + this.editor.instanceNumber, 'class': guiContentClass}).
...
// Make this editor the active editor and show it
        GUIEditor.prototype.activate = function () {
            ...
            if (this.editor.textEditor.isModified() || (
                this.editor.textEditor.isInitialized() && this.editor.xmlState.isChanged())) {
        ...
        GUIEditor.prototype.selectNode = function (selected) {
            if (!selected || selected.length == 0) {
                ...
                if (selectedObject instanceof XMLElement) {
                    ...
                    selectedObject instanceof XMLElementStub)) {
...
// Delete an element from the document and update the editor state
        GUIEditor.prototype.deleteElement = function (xmlElement) {
            ...
            if (!parent || !(
                parent instanceof XMLElement) || !parent.childCanBeRemoved(xmlElement.objectType))
                return;
...
// then select the next sibling if any are available.
        GUIEditor.prototype.selectNext = function (reverse) {
            ...
            if (this.selectedNode == null) {
                ...
                if (!(
                    selectedNode instanceof XMLElement)) {
            ...
            if (this.selectedNode == null) {
                ...
                if (selectedAttribute.length > 0) {
                    ...
                    if (this.selectedNode.domNode) {
                        selectedAttribute =
                            this.selectedNode.attributeContainer.children('.' + attributeContainerClass).first();
...
// Find and select the nearest element text field in an element or its children
        GUIEditor.prototype.focusSelectedText = function () {
            ...
            if (this.selectedNode.textInput != null) {
                ...
                focused = this.selectedNode.domNode.find(
                    "input[type=text].element_text:visible, textarea.element_text:visible, select.element_text:visible").
                    first().focus();
            }
...
// input field focused belonged to a child, then select that child.
        GUIEditor.prototype.focusInput = function (reverse) {
            ...
            if (focused.length == 0 && this.selectedNode == null) {
                ...
                focused = this.xmlContent.find(
                    "input[type=text]:visible, textarea:visible, select:visible, .edit_title:visible").first().focus();
            } else {
...
// Return true if the given dom node is vertically completely on screen
        GUIEditor.prototype.isCompletelyOnScreen = function (object) {
            ...
            return (
                docViewTop < objectTop) && (
                docViewBottom > objectBottom);
        };
...
// If the given target is not completely on screen then scroll the window to the top of the target
        GUIEditor.prototype.focusObject = function (focusTarget) {
            if (!this.isCompletelyOnScreen(focusTarget)) {
                var scrollHeight = focusTarget.offset().top + (
                    focusTarget.height() / 2) - (
                    $(window).height() / 2);
...
// Generates an individual menu entry
        MenuBar.prototype.generateMenuItem = function (menuItemData, parentMenu) {
            ...
            if (menuItemData.action != null &&
                Object.prototype.toString.call(menuItemData.action) != '[object Function]') {
            ...
            var menuBar = this;
            menuItem.data("menuItemData", menuItemData).
                attr("id", xmlMenuHeaderPrefix + menuItemData.label.replace(/ /g, "_"));
...
// Creates the structure for the menu, including headers and content areas.
        ModifyElementMenu.prototype.render = function (parentContainer) {
            ...
            this.menuContent =
                $("<ul id='" + this.menuID + "' class='" + menuContentClass + "'/>").data('menuData', this).
                    appendTo(parentContainer);
            var self = this;
...
// Populate the menu with entries for adding child elements of from the definition of the given XMLElement
        ...
        ModifyMenuPanel.prototype.initialize = function (parentContainer) {
            ...
            // Generate the document status panel, which shows a save/export button as well as if there are changes to
            // the document
            if (this.editor.options.enableDocumentStatusPanel) {
                ...
                if (self.editor.submitButtonConfigs != null) {
                    $.each(self.editor.submitButtonConfigs, function (index, config) {
                        var submitButton;
                        if (config.id && (
                            'createDomElement' in config) && !config.createDomElement) {
                        ...
                        if (!(
                            'responseHandler' in config) && config.url) {
                        ...
                        submitButton.click(function () {
                            var submitDialog = $(
                                "<form><input type='text' class='xml_export_filename' placeholder='file.xml'/><input type='submit' value='Submit'/></form>").
                                dialog({
                                    modal: true,
                                    dialogClass: 'xml_dialog',
                                    resizable: false,
                            ...
                            submitDialog.submit(function (event) {
...
// Update the position of the menu
        ModifyMenuPanel.prototype.setMenuPosition = function () {
            ...
            if ($(window).scrollTop() >= menuTop) {
                this.menuColumn.css({
                    position: 'fixed',
                    left: xmlEditorContainer.offset().left + xmlEditorContainer.outerWidth() -
                    this.menuColumn.innerWidth(),
                    top: 0
                });
            ...
            if (menuHeight + menuOffset >
                xmlWorkAreaContainer.height() + xmlWorkAreaContainer.offset().top - $(window).scrollTop()) {
                menuHeight = xmlWorkAreaContainer.height() + xmlWorkAreaContainer.offset().top - $(window).scrollTop() -
                    menuOffset;
            }
...
// Recursively walk the provided schema to construct necessary representations of the tree for the editord
        SchemaTree.prototype.build = function (elementName, elementDef, parentDef) {
            ...
            if (!elementDef.schema) {
                ...
                elementDef.name = (
                        namespaceDefinition.prefix ? namespaceDefinition.prefix + ":" : "") + elementDef.localName;
            }
            ...
            if (elementDef.attributes)
                $.each(elementDef.attributes, function () {
                    ...
                    this.name = (
                            namespaceDefinition.prefix ? namespaceDefinition.prefix + ":" : "") + this.localName;
                });
...
// defined for this element in the schema
        SchemaTree.prototype.pathMatches = function (elementNode, definition) {
            ...
            for (index in definition.parents) {
                ...
                if (isRootNode) {
                    ...
                    if (parentDef.localName == localName(parentNode) &&
                        parentDef.namespace == parentNode.namespaceURI) {
        ...
        TextEditor.prototype.initialize = function (parentContainer) {
            this.xmlContent =
                $("<div/>").attr({'id': textContentClass + this.editor.instanceNumber, 'class': textContentClass}).
                    appendTo(parentContainer);
...
// Adjust the size of the editor to reflect its environment
        TextEditor.prototype.resize = function () {
            var xmlEditorHeight = (
            $(window).height() - this.xmlEditorDiv.offset().top);
            ...
            if (this.editor.modifyMenu.menuContainer != null) {
                this.editor.modifyMenu.menuContainer.css({
                    'max-height': $(this.editor.xmlWorkAreaContainer).height() -
                    this.editor.modifyMenu.menuContainer.offset().top
                });
            }
...
// Select the tag currently encapsulating the cursor and refresh the editor to indicate this
        TextEditor.prototype.selectTagAtCursor = function () {
            if (!this.isInitialized())
                return this;
            var currentLine = this.aceEditor.getSession().getDocument().
                getLine(this.aceEditor.selection.getCursor().row);
            ...
            // Check to see if the tag being selected is already selected.  If it is and the document hasn't been
            // changed, then quit.
            if (match != null && !this.inSelectedTag(currentRow, openingIndex, closingIndex)) {
                ...
                var elementNode = this.editor.xmlState.xml[0].getElementsByTagName(tagTitle)[instanceNumber];
                if (!elementNode) {
                    elementNode = $(unprefixedTitle, this.editor.xmlState.xml).filter(function () {
...
// text in the editor and select the new tag
        TextEditor.prototype.addElementEvent = function (parentElement, newElement) {
            ...
            var pattern = new RegExp(
                "<(" + this.editor.options.targetPrefix + ":)?" + localName(newElement.xmlNode[0]) + "(\\s|\\/|>|$)",
                "g");
...
// Render the gui representation of this attribute
        XMLAttribute.prototype.render = function () {
            ...
            this.attributeInput =
                this.createElementInput(this.domNodeID.replace(":", "-"), attributeValue, this.domNode[0]);
...
// Change the attribute's value in the xml document to value
        ...
        XMLAttributeStub.prototype.render = function () {
            ...
            var createLink = $("<span class='create_attr'>create attribute</span>").appendTo(this.domNode).
                mouseup(function (e) {
                    self.create();
                });
...
// Persist the input value back into the text node
        ...
        function XMLElement(xmlNode, objectType, editor) {
            ...
            this.allowAttributes = this.objectType.anyAttribute || (
                this.objectType.attributes && this.objectType.attributes.length > 0);
...
// Returns the newly created GUI dom element
        XMLElement.prototype.render = function (parentElement, recursive, relativeTo, prepend) {
            ...
            this.domElement.className =
                this.objectType.localName + "_" + this.objectType.ns + 'Instance ' + xmlNodeClass + ' ' +
                xmlElementClass;
            if (this.isTopLevel)
        ...
        XMLElement.prototype.renderAttributes = function () {
            ...
            $(this.xmlNode[0].attributes).each(function () {
                ...
                for (var i = 0; i < attributesArray.length; i++) {
                    if (attributesArray[i].localName == attrLocalName &&
                        attributesArray[i].namespace == attrNamespace) {
...
// minimum occurrence restrictions
        XMLElement.prototype.childCanBeRemoved = function (childType) {
            ...
            if (this.presentChildren[childName] && this.objectType.occurs && childName in this.objectType.occurs)
                return (
                this.presentChildren[childName] > this.objectType.occurs[childName].min);
            return true;
        };
        ...
        XMLElement.prototype.addContentContainers = function (recursive) {
            ...
            if (this.allowText) {
                if (this.allowAttributes) {
                    if (this.allowChildren) {
                        placeholder.appendChild(
                            document.createTextNode('Use the menu to add subelements, attributes and text.'));
                    } else {
                ...
                if (this.allowAttributes) {
                    if (this.allowChildren) {
                        placeholder.appendChild(
                            document.createTextNode('Use the menu to add subelements and attributes.'));
                    } else {
        ...
        XMLElement.prototype.addElement = function (objectType, relativeTo, prepend) {
            ...
            if (xmlDocument.createElementNS) {
                newElement = xmlDocument.createElementNS(objectType.namespace, prefix + objectType.localName);
                    ...
                    xmlDocument.createNode) != "undefined") {
                // Older IE versions
...
// Add a new attribute of type objectType to this element
        ...
        XMLElement.prototype.addNode = function (nodeType, prepend, relativeTo) {
            ...
            switch (nodeType) {
                ...
                case "cdata" :
                    return this.renderCData(null, prepend, relativeTo);
                case "comment" :
                    return this.renderComment(null, prepend, relativeTo);
                case "element" :
                    if (this.allowChildren) {
...
// Inform the element that its contents have been update, so that it can refresh itself
        ...
        XMLElementStub.prototype.render = function (parentElement, prepend, relativeToXMLElement) {
            ...
            var createLink = $("<span class='create_element'>create element</span>").appendTo(elementNameContainer).
                mouseup(function (e) {
                    self.create();
                });
        ...
        XMLElementStub.prototype.create = function () {
            ...
            if (newElement instanceof AbstractXMLObject) {
                // Move new element to match display position of the stub, in case it was misplaced because of its
                // siblings being stubs
            ...
            this.form.on('keydown click', '.templating', function (e) {
                ...
                if (key === 1 || key === 9) {
                    ...
                    if (key == 1) {
                        ...
                        next_element = (
                        form_id === number_of_forms) ? 0 : form_id;
